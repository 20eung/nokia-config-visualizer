...
exports.extractSystemIp = extractSystemIp;
exports.parseQosPolicyDefinitions = parseQosPolicyDefinitions;
exports.parseSAPs = parseSAPs;
exports.parseSpokeSDP = parseSpokeSDP;
exports.parseMeshSDP = parseMeshSDP;
exports.parseEpipe = parseEpipe;
exports.parseVPLS = parseVPLS;
exports.parseVPRN = parseVPRN;
exports.parseL2VPNServices = parseL2VPNServices;
exports.extractPortInfo = extractPortInfo;
exports.extractPortDescriptions = extractPortDescriptions;
exports.parseSDPs = parseSDPs;
exports.parseL2VPNConfig = parseL2VPNConfig;
 ...
 * 호스트명 추출
 */
function extractHostname(configText) {
 ...
 * 시스템 IP 추출
 */
function extractSystemIp(configText) {
 ...
 */
function parseQosPolicy(content, direction) {
 ...
 */
function parseQosPolicyDefinitions(configText) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        for (let j = i + 1; j < lines.length; j++) {
            ...
            if (trimmed === 'exit' && indent === startIndent)
                break;
            // rate 라인 파싱
 ...
 */
function detectServiceAdminState(content) {
 ...
 */
function parseSAPs(serviceContent) {
    const saps = [];
    // 모든 SAP 시작 위치 수집
    ...
    for (let i = 0; i < sapStarts.length; i++) {
        ...
        const adminState = sapContent.includes('shutdown') && !sapContent.includes('no shutdown') ? 'down' : 'up';
        saps.push({
            sapId,
            portId,
            vlanId,
 ...
 * Spoke SDP 파싱
 */
function parseSpokeSDP(serviceContent) {
    const spokeSdps = [];
 ...
 * Mesh SDP 파싱
 */
function parseMeshSDP(serviceContent) {
    const meshSdps = [];
 ...
 * Epipe 서비스 파싱
 */
function parseEpipe(serviceId, customerId, content, serviceName) {
    // Description 추출
    const descMatch = content.match(RE_DESCRIPTION);
    const description = descMatch ? descMatch[1] : '';
 ...
 * VPLS 서비스 파싱
 */
function parseVPLS(serviceId, customerId, content, serviceName) {
    // Description 추출
    const descMatch = content.match(RE_DESCRIPTION);
    const description = descMatch ? descMatch[1] : '';
    // Service Name 추출
 ...
 * VPRN 서비스 파싱
 */
function parseVPRN(serviceId, customerId, content, serviceName) {
    // Description 추출
    const descMatch = content.match(RE_DESCRIPTION);
    const description = descMatch ? descMatch[1] : '';
    // Service Name 추출
 ...
 * L2/L3 VPN 서비스 파싱
 */
function parseL2VPNServices(configText) {
    const services = [];
...
function extractPortInfo(configText) {
    ...
    for (const line of lines) {
        ...
        if (currentPort) {
            ...
            if (inEthernet) {
                ...
                if (modeMatch && ethernet)
                    ethernet.mode = modeMatch[1];
                const encapMatch = trimmed.match(RE_ETHERNET_ENCAP);
                if (encapMatch && ethernet)
                    ethernet.encapType = encapMatch[1];
                const mtuMatch = trimmed.match(RE_ETHERNET_MTU);
                if (mtuMatch && ethernet)
                    ethernet.mtu = parseInt(mtuMatch[1]);
                const speedMatch = trimmed.match(RE_ETHERNET_SPEED);
                if (speedMatch && ethernet)
                    ethernet.speed = speedMatch[1];
                const autoMatch = trimmed.match(RE_ETHERNET_AUTO);
                if (autoMatch && ethernet)
                    ethernet.autonegotiate = autoMatch[1];
                // network sub-block: queue-policy
                const nqpMatch = trimmed.match(RE_ETHERNET_QP);
                if (nqpMatch && ethernet)
                    ethernet.networkQueuePolicy = nqpMatch[1];
                // lldp sub-block: admin-status
                const lldpMatch = trimmed.match(RE_ETHERNET_ADMIN);
                if (lldpMatch && ethernet)
                    ethernet.lldp = lldpMatch[1];
            }
        }
    }
 ...
 */
function extractPortDescriptions(configText) {
...
function parseSDPs(configText) {
    ...
    for (const match of matches) {
        ...
        const description = descMatch ? descMatch[1] : '';
        // Admin state
        const adminState = content.includes('shutdown') && !content.includes('no shutdown') ? 'down' : 'up';
        sdps.push({
            sdpId: parseInt(sdpIdStr),
            description,
            farEnd,
 ...
 * L2 VPN 설정 파싱 (메인 함수)
 */
function parseL2VPNConfig(configText) {
    ...
    services.forEach(service => {
        if ('saps' in service) {
            service.saps.forEach(sap => {
                ...
                if (info) {
                    if (info.description)
                        sap.portDescription = info.description;
                    if (info.ethernet)
                        sap.portEthernet = info.ethernet;
                }
        ...
        if ('interfaces' in service && (service.serviceType === 'vprn' || service.serviceType === 'ies')) {
            ...
            l3Service.interfaces.forEach(intf => {
                ...
                if (intf.portId) {
                    ...
                    if (info) {
                        if (info.description && !intf.portDescription)
                            intf.portDescription = info.description;
                        if (info.ethernet)
                            intf.portEthernet = info.ethernet;
                    }
                }
                // Inject QoS rate info from policy definitions
    ...
    const parsedInterfaceNames = new Set(); // 중복 방지
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
    ...
    const filteredBaseInterfaces = baseInterfaces.filter(intf => {
        ...
        if (intf.interfaceName.toLowerCase() === 'system')
            return false;
        // Must have IP address
        if (!intf.ipAddress)
            return false;
        // Must have at least one associated static route
        const hasAssociatedRoute = staticRoutes.some(route => isIpInSubnet(route.nextHop, intf.ipAddress));
    ...
    // ---------------------------------------------------------
    const explicitIesServices = services.filter(s => s.serviceType === 'ies' && s.serviceId !== 0);
    if (explicitIesServices.length > 0 && staticRoutes.length > 0) {
        for (const ies of explicitIesServices) {
            const existingKeys = new Set(ies.staticRoutes.map(r => `${r.prefix}|${r.nextHop}`));
...
//# sourceMappingURL=parserV3.js.map